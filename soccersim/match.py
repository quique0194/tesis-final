...
from soccersim.utils import sign, dist, angle_of, angle_to, angle_diff
...
class Match(object):
    ...
    def calculate_blue_score(self):
        ...
        # If kicked r=angular_diff_to_goal
        elif self.blue_team[1].kicked:
            alpha = angle_of(self.ball.last_move)
            beta = angle_to(self.ball.pos, goals[0])
            x = abs(angle_diff(alpha, beta))
            # f(x) = (1-x/180)**2 / 5
            self.blue_score = 0.3 + (1 - x / 180)**2 / 5

        # r=dist_to_goal:[0, 0.2]
        else:
            # f(x) = (2.83-x)**2 / 40
            x = dist(self.blue_team[1].pos, self.ball.pos)
            self.last_blue_score = (2.83 - x)**2 / 40
